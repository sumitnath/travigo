...
const Memory = ({ memory: { title, text, subtitle, img, experience } }) => {
  ...
  return (
    ...
    gap-16 xl:gap-9 lg:flex-col-reverse"
        ...
        lg:justify-center w-full max-w-md"
          >
            <img
        ...
        font-bold text-slate-900 filter drop-shadow-lg"
            ...
            <div 
            className=" grid items-center grid-cols-3 gap-7 xl:gap-3">
              {experience?.map((val, index) => (
                <div
                  key={index}
                  className="bg-gradient-to-b from-emerald-300 to-green-300 rounded-lg h-autow-auto
            shadow-emerald-200 flex items-center justify-center flex-col
            py-7 px-5 xl:p-5 rounded-lg text-slate-900 filter
            cursor-pointer hover:scale-105 transition-all duration-400 
            "
                >
                  <h1 className="text-3xl xl:text-2xl sm:text-xl 
